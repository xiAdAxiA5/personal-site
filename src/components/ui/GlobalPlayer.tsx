  ...
  return (
    <AnimatePresence>
      {showTopBar && track && (
        ...
        >
          <div className="pointer-events-auto rounded-2xl overflow-hidden w-full max-w-4xl
            ...
            {/* Layout: left info | center lyrics | right controls */}
            <div className={`flex items-center gap-5 px-6 ${twoLine ? 'h-22' : 'h-20'}`}>
              {/* Left: cover + track info */}
              <div className="flex items-center gap-3 shrink-0 w-[200px] cursor-pointer" onClick={() => ctx.setView('playing')}>
                <img src={selected.cover} alt={selected.title} className="w-14 h-14 rounded-xl object-cover shadow-sm shrink-0" />
                <div className="min-w-0">
                  <p className="text-base font-semibold dark:text-white truncate leading-tight">{track.title}</p>
                  <p className="text-base text-black/40 dark:text-white/30 truncate leading-tight">{selected.artist}</p>
                </div>
              </div>
