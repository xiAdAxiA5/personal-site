...
export default function GameSection() {
  ...
  const [activeCat, setActiveCat] = useState<string>('steam');
  const [steamGames, setSteamGames] = useState<SteamGame[]>([]);
  const [steamLoading, setSteamLoading] = useState(true);

  const selectedItem = watchList.find((o) => o.id === selected);
  const classicItems = watchList.filter((o) => o.category === 'game');

  useEffect(() => {
    fetch('/api/steam-games')
      .then((r) => r.json())
      .then((data) => {
        if (!data.error) setSteamGames(data);
  ...
  return (
    <section className="pt-[76px] pb-[76px] px-4">
      <div className="max-w-6xl mx-auto">
        {/* Category nav as header — same as Watch */}
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {activeCat === 'steam' ? (
              /* Steam — horizontal scrolling cards */
              <div className="overflow-x-auto scrollbar-hide pb-2">
                {steamLoading ? (
                  <div className="flex gap-3">
                    {[...Array(5)].map((_, i) => (
                      <div key={i} className="shrink-0 w-[200px] animate-pulse">
                        ...
                        <div className="mt-1 h-3 w-1/3 rounded bg-gray-100 dark:bg-gray-800" />
                      </div>
                    ))}
                  </div>
                ) : steamGames.length > 0 ? (
                  ...
                  <div className="flex items-center justify-center min-h-[200px]">
                    <p className="text-xs text-gray-300 dark:text-gray-600 italic">连接 Steam 以同步游戏时长</p>
                  </div>
                )}
              </div>
            ) : (
              /* Classic — grid same as MediaGrid */
